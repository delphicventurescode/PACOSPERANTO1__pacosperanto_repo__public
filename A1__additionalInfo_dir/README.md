## additionalInfo_dir

This directory has additional information on the Pacosperato language. This information is provided only in English.

[+]

### SECTION 1

[+]

#linguistics #IPLaw

Comments & additional info on the Pacosperanto Language:

1. Information on the language referred to as Pacosperanto will be provided on a "need to know" basis.

2. Anyone who speaks Esperanto already speaks Pacosperanto because every sentence in Esperanto is a valid sentence in Pacosperanto.

3. A patent has been applied for on technologies surrounding the language (patent held by Prof. Anand Manikutty, and Prof. Sankaran Manikutty), but it is envisioned that the technologies will be made available to everyone for free. The patent application process has been carefully conducted to achieve such an outcome.

4. Please note that literacy improvement is actually a secondary goal. The primary goal is to create a KFP (Komunumo De Pacfarantoj), or Community of Peacemakers.

5. Please also note that this is an experimental project.

[+]

### SECTION 2

[+]

#### URL for the paper

URL for additional information on Pacosperanto, courtesy Google Books:

https://books.google.co.in/books/about/ESPERANTANANDA.html?id=eYDozwEACAAJ&redir_esc=y

[+]

#### Information on the paper

The full paper has been uploaded to SSRN. Reading the paper will give you all the information you need about the Pacosperanto Language.

#### Abstract:

In this paper, we introduce a new language called Esperantananda. Also sometimes called Pacosperanto, Hindu-Esperanto, Hesperanto, Esparantanando, Termite-speranto or Anand-speranto, Esperantananda is a natural language that is designed to be very easy to learn. Speakers of Hindi can pick up the basics of this language in around two weeks. Designed for a South Asian audience, people are able to start speaking Hesperanto within two weeks of learning provided they know Sanskrit or Hindi and, preferably, English. The English requirement comes in purely because the Ĉirkaŭaj materialoj (trans. : "surrounding materials") are all in English. Learning the language does not require any knowledge of English, in actuality. The language has two cases - nominative and accusative - similar to Esperanto. All nouns end in -o, all adjectives end in -a and all infinitives end in -I, similar to Esperanto. In addition, English words that are technology-related, especially products and companies, are adopted as such in Anand-speranto with an -x ending. Furthermore, a large number of nouns, verbs and adjectives are adopted from Hindi or Hindustani (originally Sanskrit). The Sanskrit-nisht form of Hindi is favored since the Urdu-based version of Hindustani has words from multiple sources, including Turkish, Arabic and Persian. It is expected that this language can significantly help with literacy efforts in the Indian subcontinent where literacy rates are still relatively low by world standards. It is also meant to be a language free from racial and ethnic slurs and insults. Every sentence in Esperanto is a valid sentence in this language, and so the number of speakers of this language is estimated to be between 100,000 and 2 million, depending on the estimation methodology. This language is also designed to create a community of peacemakers around the world. Thereby, this language is designed to promote world peace.

[+]

#### Additional note in a somewhat humorous vein


The abstract on Google Books is a bit out of date, but it still gives you the basic blueprint of how the language works. For the full, unvarnished story, check out our patent filing documents (available upon request). As for why this language has more names than a royal newborn? We accumulated so much code over time that different developers and contributors just started making up their own names for it—partly out of affection, but mostly out of chaos.

[+]
